...
allSubj_Answers = [ 2 1 1 1 1 1 1 1 1];
...
NrOfRuns            = ones(numel(SubjNames),1).*9; % excluding localizer run
...
%         prt_file = xff('*.prt');
        ...
        for lengthRunTrials=0:NrOfRep-1 % this will reduce the number of trials in a run
            ...
            allSubj_DecisionsA(5,1:9) = allDecisionMat(:,10,BestChannelsID_a(5));
